...
public interface ScoreItemRepository extends CrudRepository<ScoreItem,Integer> {
    @Query("select s from ScoreItem s where s.score.courseClass.ccNumber=?1 and s.experimentName=?2")
    List<ScoreItem> findScoreItemsByCcNumberAndExperienceName(String ccNumber, String experienceName);
    @Query("select s from ScoreItem s where s.score.courseClass.ccId=?1 and s.experimentName=?2")
    List findScoreItemsByCcNumberAndExperimentName(int ccId,String experimentName);
